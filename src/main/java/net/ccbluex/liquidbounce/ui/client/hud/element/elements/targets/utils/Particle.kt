...
import net.ccbluex.liquidbounce.utils.render.SKIDRenderUtils
...
class Particle(var color: Color, var distX: Float, var distY: Float, var radius: Float, var drawType: ShapeType = ShapeType.SOLID_CIRCLE) {
    ...
    fun render(x: Float, y: Float, fade: Boolean, speed: Float, fadeSpeed: Float, canRotate: Boolean = false) {
        if (progress >= 1.0) {
            progress = 1.0
            if (fade) alpha -= (fadeSpeed * 0.02F * SKIDRenderUtils.deltaTime)
            if (alpha < 0F) alpha = 0F
        } else
            progress += (speed * 0.025F * SKIDRenderUtils.deltaTime).toDouble()
...
enum class ShapeType(val typeName: String) {
    SOLID_CIRCLE("c_solid") {
        override fun performRendering(x: Float, y: Float, rad: Float, col: Color) {
            SKIDRenderUtils.drawFilledCircle(x, y, rad, col)
        }
    },
    CIRCLE("c_outline") {
        override fun performRendering(x: Float, y: Float, rad: Float, col: Color) {
            SKIDRenderUtils.drawCircle(x, y, rad, 0.5F, 0, 360, col)
        }
    },
    SOLID_RECT("r_solid") {
        override fun performRendering(x: Float, y: Float, rad: Float, col: Color) {
            SKIDRenderUtils.drawRect(x - rad / 2F, y - rad / 2F, x + rad / 2F, y + rad / 2F, col.rgb)
        }
    },
    RECT("r_outline") {
        override fun performRendering(x: Float, y: Float, rad: Float, col: Color) {
            SKIDRenderUtils.drawBorder(x - rad / 2F, y - rad / 2F, x + rad / 2F, y + rad / 2F, 0.5F, col.rgb)
        }
    },
    SOLID_TRIANGLE("t_solid") {
        override fun performRendering(x: Float, y: Float, rad: Float, col: Color) {
            SKIDRenderUtils.drawTriAngle(x, y, rad, 3F, col, true)
        }
    },
    TRIANGLE("t_outline") {
        override fun performRendering(x: Float, y: Float, rad: Float, col: Color) {
            SKIDRenderUtils.drawTriAngle(x, y, rad, 3F, col, false)
        }
    };
